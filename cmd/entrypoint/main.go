...
func run() int {
	...
	if _, err = os.Stat(*ipoibBinFile); err != nil { //nolint:gosec
...
func copyFileAtomic(srcFilePath, destDir, tempFileName, destFileName string) error {
	...
	srcFile, err := os.Open(srcFilePath) //nolint:gosec
	if err != nil {
	...
	srcFileStat, err := os.Stat(srcFilePath) //nolint:gosec
	if err != nil {
		return err
	}

	if err := os.Chmod(f.Name(), srcFileStat.Mode()); err != nil { //nolint:gosec
